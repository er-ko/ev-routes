    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                ...
                <form method="POST" action="{{ route('route.update', $route) }}">
                    @csrf
                    @method('patch')

                    <input type="hidden" id="form-name" name="name" value="{{ $route->name }}" />
                    <input type="hidden" id="form-currency" name="currency" value="{{ $route->currency }}" />
                    <input type="hidden" id="form-distance" name="unit_distance" value="{{ $route->unit_distance }}" />
                    <input type="hidden" id="form-consumption" name="unit_consumption" value="{{ $route->unit_consumption }}" />
                    <input type="hidden" id="form-weight" name="unit_weight" value="{{ $route->unit_weight }}" />
                    <input type="hidden" id="form-temperature" name="unit_temperature" value="{{ $route->unit_temperature }}" />
                    <div class="grid gap-0 lg:gap-8 grid-cols-1 lg:grid-cols-3 mb-4 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="mb-8 lg:mb-0 p-4 sm:p-8 flex flex-col justify-center items-center bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                            <div class="w-full mb-4">
                                ...
                                >
                                    @foreach ($drivers as $driver)
                                        <option value="{{ $driver->id }}" {{ $route->driver_id == $driver->id ? 'selected' : '' }}>{{ $driver->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="w-full mb-4">
                                ...
                                <x-text-input id="drive-date" name="drive_date" type="date" class="mt-1 block w-full" value="{{ Carbon\Carbon::parse($route->drive_date)->format('Y-m-d') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('drive_date')" />
                            </div>
                            <div class="w-full mb-4">
                                <x-input-label for="avg-temp">{{ __('messages.route.average_temperature') }} <small class="uppercase">(°{{ $route->unit_temperature }})</small></x-input-label>
                                <x-text-input id="avg-temp" name="avg_temp" type="number" class="mt-1 block w-full" min="-90" max="90" step="1" placeholder="0" value="{{ $route->avg_temp }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('avg_temp')" />
                            </div>
                            <div class="w-full mb-4">
                                <x-input-label for="from">{{ __('messages.route.from_place') }}</x-input-label>
                                <x-text-input id="from" name="from" type="text" class="mt-1 block w-full" value="{{ $route->from }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('from')" />
                            </div>
                            <div class="w-full mb-4">
                                <x-input-label for="to">{{ __('messages.route.to_place') }}</x-input-label>
                                <x-text-input id="to" name="to" type="text" class="mt-1 block w-full" value="{{ $route->to }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('to')" />
                            </div>
                            <div class="w-full mb-4">
                                <x-input-label for="distance">{{ __('messages.route.distance') }} <small>({{ $route->unit_distance }})</small></x-input-label>
                                <x-text-input id="distance" name="distance" type="number" class="mt-1 block w-full" min="1" step="1" placeholder="0" value="{{ $route->distance }}" required />
                            ...
                            <div class="w-full mb-4">
                                ...
                                <x-text-input id="driving-time" name="driving_time" type="number" class="mt-1 block w-full" min="1" step="1" placeholder="0" value="{{ $route->driving_time }}" required />
                            ...
                            <div class="w-full">
                                ...
                                <x-text-input id="map-link" name="map_link" type="text" class="mt-1 block w-full" value="{{ $route->map_link }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('map_link')" />
                                <div class="mt-1 text-end"><a href="https://www.google.com/maps/" target="_blank" class="text-sm underline text-slate-500">google maps</a></div>
                            </div>
                        </div>
                        ...
                        <div class="col-span-2 flex flex-col justify-center items-center">
                            <div class="mb-4 p-4 sm:px-8 sm:py-12 w-full flex flex-col justify-between items-center bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                                            ...
                                            <div class="w-full mb-4">
                                                ...
                                                >
                                                    @foreach ($garages as $garage)
                                                        <option value="{{ $garage->id }}" {{ $route->garage_id == $garage->id ? 'selected' : '' }}>{{ $garage->nickname }}</option>
                                                    @endforeach
                                                </select>
                                                <x-input-error class="mt-2" :messages="$errors->get('garage_id')" />
                                            </div>
                                            <div class="w-full mb-4">
                                                <x-input-label for="avg-speed">{{ __('messages.route.average_speed') }} <small>({{ $route->unit_distance == 'mi' ? 'mph' : 'km/h' }})</small></x-input-label>
                                                <x-text-input id="avg-speed" name="avg_speed" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->avg_speed }}" required />
                                                <x-input-error class="mt-2" :messages="$errors->get('avg_speed')" />
                                            </div>
                                        </div>
                                        <div>
                                            <div class="w-full mb-4">
                                                <x-input-label for="avg-consumption">{{ __('messages.route.average_consumption') }} <small>(kWh/100)</small></x-input-label>
                                                <x-text-input id="avg-consumption" name="avg_consumption" type="number" class="mt-1 block w-full" step="0.01" placeholder="0" value="{{ $route->avg_consumption }}" required />
                                            ...
                                            <div class="w-full mb-4">
                                                ...
                                                <x-text-input id="total-consumption" name="total_consumption" type="number" class="mt-1 block w-full" step="0.01" placeholder="0" value="{{ $route->total_consumption }}" required />
                                ...
                                <div class="w-full grid gap-8 grid-cols-3">
                                    <div class="w-full mb-4">
                                        <x-input-label for="loaded-weight">{{ __('messages.route.loaded_weight') }} <small>({{ $route->unit_weight }})</small></x-input-label>
                                        <x-text-input id="loaded-weight" name="loaded_weight" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->loaded_weight }}" required />
                                    ...
                                    <div class="w-full mb-4">
                                        ...
                                        <select
                                            ...
                                            class="mt-1 block w-full py-2 px-3 border-gray-300/75 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-teal-600 focus:ring-indigo-500 dark:focus:ring-teal-600 rounded-md shadow-sm"
                                        >
                                            <option value="0" {{ !$route->trailer ? 'selected' : '' }}>{{ __('messages.no') }}</option>
                                            <option value="1" {{ $route->trailer ? 'selected' : '' }}>{{ __('messages.yes') }}</option>
                                        </select>
                                        <x-input-error class="mt-2" :messages="$errors->get('trailer')" />
                                    </div>
                                    <div class="w-full">
                                        <x-input-label for="trailer-weight">{{ __('messages.route.trailer_weight') }} <small>({{ $route->unit_weight }})</small></x-input-label>
                                        <x-text-input id="trailer-weight" name="trailer_weight" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->trailer_weight }}" required />
                                        <x-input-error class="mt-2" :messages="$errors->get('trailer_weight')" />
                                        <p class="text-xs text-right lowercase italic mt-1 pr-2 text-slate-600 dark:text-gray-400">{{ __('messages.cargo') .' + '. __('messages.trailer') }}</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="flex-1 grid gap-8 grid-cols-2">
                                    <div>
                                        <div class="w-full mb-4">
                                            ...
                                            <x-text-input id="charging-stops" name="charging_stops" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->charging_stops }}" required />
                                        ...
                                        <div class="w-full mb-4">
                                            ...
                                            <x-text-input id="charging-time" name="charging_time" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->charging_time }}" required />
                                        ...
                                        <div class="w-full mb-4">
                                            ...
                                            <x-text-input id="supplied-energy" name="supplied_energy" type="number" class="mt-1 block w-full" step="1" placeholder="0" value="{{ $route->supplied_energy }}" required />
                                    ...
                                    <div>
                                        <div class="w-full mb-4">
                                            ...
                                            <x-text-input id="price-per-kw" name="price_per_kw" type="number" class="mt-1 block w-full" step="1" placeholder="0" step="0.01" value="{{ $route->price_per_kw }}" required />
                                        ...
                                        <div class="w-full mb-4">
                                            ...
                                            <x-text-input id="real-price" name="real_price" type="number" class="mt-1 block w-full" step="1" placeholder="0" step="0.01" value="{{ $route->real_price }}" required />
                                        ...
                                        <div class="w-full">
                                            ...
                                            <x-text-input id="estimated-price" name="estimated_price" type="number" class="mt-1 block w-full" step="1" placeholder="0" step="0.01" value="{{ $route->estimated_price }}" required />
                    ...
                    <div class="grid gap-6 sm:gap-8 grid-cols-1 lg:grid-cols-2 mt-8 lg:mt-0">
                        <div>
                            ...
                            <x-textarea id="map-iframe" name="map_iframe" placeholder="{{ __('messages.route.insert_a_new_one_if_it_needs_to_be_changed') }}"></x-textarea>
                            <x-input-error :messages="$errors->get('map_iframe')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="note">{{ __('messages.route.note') }}</x-input-label>
                            <x-textarea id="note" name="note" :value="$route->note"></x-textarea>
                            <x-input-error :messages="$errors->get('note')" class="mt-2" />
                        </div>
                    </div>
                    <div class="flex justify-end items-center mt-8">
                        <div class="checkbox-wrapper-toggle me-4 lowercase">
                            <input type="checkbox" id="toggle" name="public"  {{ $route->public ? 'checked' : '' }} />
    ...
    <x-modal name="setting-area">
        <div class="p-8">
            ...
            <div class="mb-4">
                ...
                >
                    <option value="mi" {{ $route->unit_distance == 'mi' ? 'selected' : '' }}>{{ __('messages.units.miles') }} (mi)</option>
                    <option value="km" {{ $route->unit_distance == 'km' ? 'selected' : '' }}>{{ __('messages.units.kilometers') }} (km)</option>
                </select>
            </div>
            <div class="mb-4">
                ...
                <select
                    ...
                    class="mt-1 block w-full py-2 px-3 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-teal-600 focus:ring-indigo-500 dark:focus:ring-teal-600 rounded-md shadow-sm"
                >
                    <option value="Wh" {{ $route->unit_consumption == 'Wh' ? 'selected' : '' }}>Wh</option>
                    <option value="kWh" {{ $route->unit_consumption == 'kWh' ? 'selected' : '' }}>kWh</option>
                </select>
            </div>
            <div class="mb-4">
                ...
                <select
                    ...
                    class="mt-1 block w-full py-2 px-3 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-teal-600 focus:ring-indigo-500 dark:focus:ring-teal-600 rounded-md shadow-sm"
                >
                    <option value="lb" {{ $route->unit_weight == 'lb' ? 'selected' : '' }}>{{ __('messages.units.pounds') }} (lb)</option>
                    <option value="kg" {{ $route->unit_weight == 'kg' ? 'selected' : '' }}>{{ __('messages.units.kilograms') }} (kg)</option>
                </select>
            </div>        
            <div class="mb-4">
                ...
                <select
                    ...
                    class="mt-1 block w-full py-2 px-3 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-teal-600 focus:ring-indigo-500 dark:focus:ring-teal-600 rounded-md shadow-sm"
                >
                    <option value="F" {{ $route->unit_temperature == 'F' ? 'selected' : '' }}>{{ __('messages.units.fahrenheit') }} (°F)</option>
                    <option value="C" {{ $route->unit_temperature == 'C' ? 'selected' : '' }}>{{ __('messages.units.celsius') }} (°C)</option>
                </select>
            </div>
            <div class="mb-4">
                ...
                >
                    @foreach($currencies as $currency)
                        <option value="{{ $currency->iso_4217 }}" {{ $route->currency == $currency->iso_4217 ? 'selected' : '' }}>{{ $currency->iso_4217 }} ({{ $currency->symbol }})</option>
                    @endforeach
                </select>
            </div>
